...
- **Network:** it only talks to `https://poe.ninja` (prices and leagues), over TLS with normal certificate checks. It sends nothing about you or your stash; the only thing in the requests is the league name and the app version (in the User-Agent).
...
Examples of what is in scope: the app sending input to a window other than the game, data from poe.ninja or from the files in `%APPDATA%\PoeStashPricer\` making the app run code or write outside that folder, and anything that leaks clipboard contents or screenshots.
